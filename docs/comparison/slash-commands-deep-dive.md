...
| 上下文压缩 | **Kimi CLI** | `/compact` 支持自定义保留重点 |
...
| **Claude Code** | 对话式代理 | 少量精选命令（15），复杂操作通过自然语言 |
| **Gemini CLI** | 全面可配置 | 中等命令量（41），策略引擎驱动 |
| **Kimi CLI** | 双模式交互 | 精简命令（20），Shell 模式补充 |
| **Qwen Code** | 继承 Gemini + 中文优化 | 继承命令体系（23），保持兼容 |
| **Copilot CLI** | GitHub 原生，极简 | 最少命令（5），依赖 GitHub 生态 |
| **Codex CLI** | 安全第一 | 无斜杠命令，CLI 参数 + 沙箱 |
| **Goose** | MCP 原生 | CLI 子命令，MCP 驱动一切 |
| **Cursor** | IDE 原生 | GUI 交互，CLI 是辅助 |

> **核心洞察：** 命令数量不等于能力强弱。Claude Code 用 15 个命令 + 自然语言覆盖了大部分场景；Aider 用 42 个命令提供最细粒度的控制；Codex CLI 用 0 个斜杠命令 + 强制沙箱实现了最高安全性。选择取决于你偏好的交互范式。
