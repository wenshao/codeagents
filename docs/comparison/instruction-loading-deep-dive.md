...
>
> **最后更新**：2026-04-17（新增 PR#3087 Auto-Memory/Auto-Dream 已合并 + PR#3339 `.qwen/rules/` 路径规则开发中）
...
|------|------------|-----------|
| **指令文件名** | `CLAUDE.md`, `CLAUDE.local.md`, `.claude/rules/*.md` | `QWEN.md`, `AGENTS.md`（可配置），**`.qwen/rules/*.md`**（[PR#3339](https://github.com/QwenLM/qwen-code/pull/3339) open） |
| **层级数** | 6 层（Managed/User/Project/Local/AutoMem/TeamMem） | 3 层（Global/Home/Project）**+ rules 目录（PR#3339 open）+ AutoMem（PR#3087 ✓ 合并）** |
| **@include 指令** | ✅ 5 层嵌套，循环检测，路径验证 | ✅ 5 层嵌套，循环检测，路径验证 |
| **Frontmatter 路径过滤** | ✅ `paths:` glob 模式，条件规则 | 🚧 [PR#3339](https://github.com/QwenLM/qwen-code/pull/3339)（open） |
| **HTML 注释剥离** | ✅ | 🚧 [PR#3339](https://github.com/QwenLM/qwen-code/pull/3339)（open，PR 范围内包含） |
| **条件规则（按文件路径）** | ✅ `.claude/rules/*.md` + `paths:` frontmatter | 🚧 [PR#3339](https://github.com/QwenLM/qwen-code/pull/3339)（open，`.qwen/rules/*.md` + `paths:` frontmatter） |
...
### 3.6 `.qwen/rules/` 路径规则（PR#3339 开发中）

**来源**：[PR#3339](https://github.com/QwenLM/qwen-code/pull/3339)（open，2026-04-16）
...
| 维度 | Claude Code | Qwen Code |
|------|------------|-----------|
| 层级数 | 6（含 AutoMem + TeamMem） | 3 + rules（PR#3339） + AutoMem（PR#3087 ✓） |
| 文件名 | 固定（CLAUDE.md / CLAUDE.local.md） | 可配置 |
| @include 深度 | 5 | 5 |
| @include 格式 | 单一（内联） | 两种（Tree / Flat） |
| HTML 注释剥离 | ✅ | 🚧 PR#3339（open） |
| Frontmatter | ✅ paths + description + allowed-tools | 🚧 PR#3339（paths + description） |
| 条件规则 | ✅（.claude/rules/*.md + paths: glob） | 🚧 PR#3339（.qwen/rules/*.md + paths: glob，**含嵌套子目录**） |
| 急/惰加载 | ✅（无 paths 急加载，有 paths 惰加载） | 🚧 PR#3339（设计一致） |
...
| [PR#3339](https://github.com/QwenLM/qwen-code/pull/3339) | 🚧 OPEN | 2026-04-16 | `.qwen/rules/` 条件规则 + `paths:` frontmatter + HTML 注释剥离 + 递归扫描 + 嵌套子目录 |

**合并后本 Deep-Dive 的"逐维度对比"将从 13 个差距项减少到 5-6 个**（剩余差距主要是：固定文件名、Team Memory、Hook 事件、Worktree 去重、多级信任模型）。

Qwen Code 和 Claude Code 在"指令加载"这个维度上的追赶是 **2026 年 4 月最显著的功能对齐**之一，Qwen Code 借助 PR#3087 + PR#3339 基本完成了对 Claude Code 指令系统核心能力的对标。
