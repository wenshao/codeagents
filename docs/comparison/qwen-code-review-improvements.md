...
> 基于 Qwen Code、Gemini CLI、Claude Code、Copilot Code Review、gstack 五方源码/架构对比，提出 Qwen Code `/review` 功能的改进方向。
>
> **实施进展**：[PR#2932](https://github.com/QwenLM/qwen-code/pull/2932) ✓ 已合并——实现了本报告 10 项建议中的 **9 项**（P0 全部 + P1 全部 + P2 大部分 + P3 全部）。仅"异步后台审查"未实现。
>
> 返回 [改进建议总览](./qwen-code-improvement-report.md) | [/review 命令对比](./review-command.md)

## 一、Qwen Code /review 的架构

> **注**：以下为 [PR#2932](https://github.com/QwenLM/qwen-code/pull/2932) 合并后的最新架构（2026-04-09），SKILL.md 从 261 行扩展到 513 行。
...
**关键改进**（对比旧版 4 Agent 架构）：

| 能力 | 旧版 | PR#2932 后 | 对标 |
|------|------|-----------|------|
| 确定性分析 | ❌ 纯 LLM | ✅ 6 语言 linter/typecheck 内置 | Copilot CodeQL |
| 构建测试 | ❌ 未验证 | ✅ Agent 5 并行执行 | Copilot CI |
| 跨文件影响 | ❌ 仅看 diff | ✅ grep 调用者 + 签名兼容检查 | Gemini codebase_investigator |
| 自定义规则 | ❌ 无 | ✅ .qwen/review-rules.md | Copilot copilot-instructions.md |
| Autofix | ❌ 文字建议 | ✅ 用户确认 → 自动应用 + 验证 + commit | Copilot Autofix |
| 评论聚合 | ❌ 重复报告 | ✅ 同类合并 + >5 条显示 top 3 | Claude /ultrareview |
| 置信度 | ❌ 二元 | ✅ High/Low，Low 仅终端显示 | Claude /ultrareview |
| PR 评论控制 | ❌ 全部发布 | ✅ 仅高置信 Critical/Suggestion | Copilot "Silence > noise" |
| 增量审查 | ❌ 每次全量 | ✅ SHA + model 缓存 | Copilot 新 commit 触发 |
| 报告持久化 | ❌ 终端关闭丢失 | ✅ .qwen/reviews/ Markdown | Gemini assessment |
| 反向审计 | ❌ 无 | ✅ 专门 Agent 发现覆盖盲区 | — |
| Worktree 隔离 | ❌ stash/checkout | ✅ 临时 worktree | Gemini 临时 worktree |
| PR 评论去重 | ❌ 无 | ✅ 获取已有评论避免重复 | — |
| 批量验证 | ❌ N 个 Agent | ✅ 单 Agent 批量验证 | — |
| 跨仓库 PR | ❌ 不支持 | ✅ `gh pr diff <url>` 轻量模式 | — |
...
4. **Qwen Code（`/review` Skill，PR#2932 后）**：
...
| 能力 | Qwen Code（PR#2932 后） | Copilot Code Review | Claude Code | Gemini CLI |
|------|-----------|-------------------|-------------|-----------|
...
> PR#2932 之前的 8 项差距中，**7 项已补齐**（✅），仅剩 1 项（❌）。

| # | 差距 | PR#2932 前 | PR#2932 后 |
|:-:|------|:----------:|:----------:|
| 1 | 只靠 LLM，无确定性分析 | ❌ | ✅ 6 语言 linter/typecheck |
| 2 | 不运行构建和测试 | ❌ | ✅ Agent 5 并行执行 |
| 3 | 不分析跨文件影响 | ❌ | ✅ grep 调用者 + 签名检查 |
| 4 | 无项目自定义规则 | ❌ | ✅ .qwen/review-rules.md |
| 5 | 同步阻塞 | ❌ | ❌ **仍未解决** |
| 6 | 缺少 Autofix 闭环 | ❌ | ✅ 用户确认 + 自动应用 |
| 7 | 评论缺乏聚合与置信度 | ❌ | ✅ 同模式合并 + High/Low |
| 8 | 缺乏记忆与持久化 | ❌ | ✅ 增量缓存 + .qwen/reviews/ |

**剩余差距**：
1. **异步后台审查**——审查大 PR 时仍阻塞 CLI 2-3 分钟。需要 `BundledSkillLoader` 支持后台执行。
2. **跨 PR 记忆**——Copilot 独有，识别的代码模式可在后续审查中复用。
3. **用户反馈闭环**——Copilot 的 👍/👎 + 原因反馈，直接用于模型迭代。
4. **LLM 信任边界审查**——gstack `/review` 的特色维度，检测 prompt injection 和用户输入直传 LLM。

## 四、Qwen Code /review 基础设施

> PR#2932 后，以下基础设施均已在 SKILL.md 中实际使用。
...
> ✅ = [PR#2932](https://github.com/QwenLM/qwen-code/pull/2932) 已实现（2026-04-09 合并）
...
> [PR#2932](https://github.com/QwenLM/qwen-code/pull/2932) ✓ 已合并（2026-04-09），一次性实现了 10 项建议中的 **9 项**。

| 优先级 | 改进 | 状态 | 实现方式 | 对标 |
|--------|------|:----:|---------|------|
| **P0** | 集成 linter/typecheck | ✅ **已实现** | 6 语言内置 + CI 配置自动发现 | Copilot CodeQL |
| **P1** | 构建 & 测试执行 | ✅ **已实现** | Agent 5 并行执行 | Copilot CI |
| **P1** | 跨文件影响分析 | ✅ **已实现** | grep 调用者 + 签名兼容检查 | Gemini codebase_investigator |
| **P1** | 项目自定义审查规则 | ✅ **已实现** | .qwen/review-rules.md + copilot-instructions.md | Copilot copilot-instructions.md |
| **P2** | 自动修复闭环（Autofix） | ✅ **已实现** | 用户确认 → 自动应用 + 验证 + commit | Copilot Autofix |
| **P2** | 评论聚合 | ✅ **已实现** | 同类合并 + >5 条显示 top 3 | Claude /ultrareview |
| **P2** | 异步后台审查 | ❌ **未实现** | — | Gemini async-pr-review |
| **P2** | Severity 置信度 | ✅ **已实现** | High/Low，Low 仅终端显示 | Claude /ultrareview |
| **P3** | 增量审查 | ✅ **已实现** | SHA + model 缓存 | Copilot 新 commit 触发 |
| **P3** | 报告持久化 | ✅ **已实现** | .qwen/reviews/ Markdown | Gemini assessment |

**PR#2932 额外实现了报告未建议的 6 项改进**：

| 额外改进 | 说明 |
|---------|------|
| 反向审计 | 专门 Agent 发现覆盖盲区 |
| Worktree 隔离 | 临时 worktree 替代 stash/checkout，不碰用户工作区 |
| PR 评论去重 | 获取已有 PR 评论避免重复报告 |
| 批量验证 | 单 Agent 批量验证所有 findings（非 N 个 Agent） |
| 跨仓库 PR | `gh pr diff <url>` 轻量模式 |
| PR 安全加固 | 审查规则从 base branch 读取（防注入）+ PR 描述标记为 DATA |

> **关键洞察**：PR#2932 证实了本报告的核心判断——P0-P1 改进确实只需修改 `SKILL.md`（从 261 行到 513 行），仅 `BundledSkillLoader.ts` 增加了 5 行代码（`{{model}}` 模板变量）。Qwen Code skill 架构的优势得到验证。
>
> **设计哲学锚点**：PR#2932 严格遵循了 Copilot 的 "Silence is better than noise" 原则——linter 警告不发 PR 评论，不确定的问题不报告，低置信度 findings 仅终端显示。

## 七、剩余差距与下一步

PR#2932 实现后，Qwen Code `/review` 与竞品的主要差距收窄为：

| 差距 | 竞品参考 | 难度 | 优先级 |
|------|---------|:----:|:------:|
| **异步后台审查** | Gemini CLI `async-pr-review` | 中（需 BundledSkillLoader 改动） | P2 |
| **LLM 信任边界审查** | gstack /review（检测 prompt injection + 用户输入直传 LLM） | 小（SKILL.md 加维度） | P2 |
| **Proactive 触发** | gstack（检测到 push/merge 时主动建议审查） | 小（SKILL.md 描述） | P3 |
| **审查→QA 联动** | gstack /review → /qa 浏览器验证前端变更 | 中（需 browse 能力） | P3 |
| **远程云端审查** | Claude Code /ultrareview（5-20 Agent fleet，10-25 分钟） | 大（需云端基础设施） | P3 |
