...
### 36. useInputBuffer 输入缓冲 Hook（P3）

**做什么**：Claude Code 的 useInputBuffer Hook——输入缓冲管理：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/useInputBuffer.ts` | 132 | 输入缓冲 |

**Qwen Code 现状**：Qwen Code **没有输入缓冲 Hook**。

**Qwen Code 修改方向**：新建 `hooks/useInputBuffer.ts`。
...
### 37. useIssueFlagBanner 问题标志 Banner（P3）

**做什么**：Claude Code 的 useIssueFlagBanner Hook——显示问题标志：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/useIssueFlagBanner.ts` | 133 | 问题标志显示 |

**Qwen Code 现状**：Qwen Code **没有问题标志功能**。

**Qwen Code 修改方向**：新建 `hooks/useIssueFlagBanner.ts`。
...
### 38. useDiffData 差异数据 Hook（P3）

**做什么**：Claude Code 的 useDiffData Hook——差异数据处理：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/useDiffData.ts` | 110 | 差异数据处理 |

**Qwen Code 现状**：Qwen Code **没有专门的差异数据 Hook**。

**Qwen Code 修改方向**：新建 `hooks/useDiffData.ts`。
