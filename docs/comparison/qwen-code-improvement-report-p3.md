...
### 12. 沙箱排除命令 excludedCommands（P3）

**做什么**：Claude Code 允许用户配置一组命令模式，这些命令**不被沙箱执行**——直接在宿主机运行：

```json
{ "sandbox": { "excludedCommands": ["docker:*", "npm run test:*", "bazel:*"] } }
```
...
| 文件 | 行数 | 职责 |
|------|------|------|
| `commands/sandbox-toggle/sandbox-toggle.tsx` | 132 | `/sandbox` 命令 + `exclude` 子命令 |
| `utils/sandbox/sandbox-adapter.ts` | ~900 | `addToExcludedCommands()` 函数 |
| `tools/BashTool/shouldUseSandbox.ts` | ~100 | excludedCommands 匹配逻辑 |
| `components/sandbox/SandboxSettings.tsx` | 295 | 沙箱设置 UI（3 种模式） |
| `components/sandbox/SandboxOverridesTab.tsx` | 192 | excludedCommands 展示 |

**总规模**：~1700 行

**Qwen Code 现状**：Qwen Code 的 `sandbox.ts`（984 行）支持 docker/podman/sandbox-exec，但**没有 excludedCommands 功能**，也没有交互式沙箱设置 UI。

**Qwen Code 修改方向**：
1. 在 `settingsSchema.ts` sandbox 配置中添加 `excludedCommands` 字段
2. 新建 `utils/sandbox/excludedCommands.ts`——命令模式匹配逻辑
3. 修改 `shouldUseSandbox()` 逻辑——检查命令是否匹配 excludedCommands
4. 扩展 `/sandbox` 命令——添加 excludedCommands 管理 UI
...
### 13. 交互式隐私设置对话框 /privacy-settings（P3）

**做什么**：Claude Code 的 `/privacy-settings` 命令提供**交互式对话框**，让用户切换 "Help improve Claude" 设置：

```
Data Privacy
━━━━━━━━━━━━
Help improve Claude    true/false  ← Enter/Tab/Space 切换
```
...
| 文件 | 行数 | 职责 |
|------|------|------|
| `commands/privacy-settings/privacy-settings.tsx` | 71 | 命令入口 + Grove 资格检查 |
| `components/grove/Grove.tsx` | 462 | `GroveDialog` + `PrivacySettingsDialog` 组件 |
| `services/api/grove.ts` | 348 | Grove API——`getGroveSettings()`、`updateGroveSettings()` |

**总规模**：~893 行

**Qwen Code 现状**：Qwen Code 有 `privacy` 配置 schema（`usageStatisticsEnabled` 字段），但**没有交互式对话框**。

**Qwen Code 修改方向**：
1. 新建 `components/PrivacySettingsDialog.tsx`
2. 修改 `/settings` 命令——添加隐私设置快捷入口
...
### 14. 自动发布说明 /release-notes（P3）

**做什么**：Claude Code 的 `/release-notes` 命令自动从 GitHub 获取并显示发布说明：
...
| 文件 | 行数 | 职责 |
|------|------|------|
| `commands/release-notes/release-notes.ts` | 61 | 命令入口 |
| `utils/releaseNotes.ts` | 370 | 获取、解析、缓存、显示 |

**总规模**：~443 行

**Qwen Code 现状**：Qwen Code 有 `/status` 显示版本信息，但**没有 release notes 功能**。

**Qwen Code 修改方向**：
1. 新建 `utils/releaseNotes.ts`
2. 新建 `/release-notes` 命令
3. 启动时后台获取 changelog
...
### 15. 企业用量管理 /extra-usage（P3）

**做什么**：Claude Code 的 `/extra-usage` 命令让企业/团队用户管理额外 API 用量——检查用量、向管理员申请增加：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `commands/extra-usage/extra-usage-core.ts` | 118 | 用量管理核心——检查资格、申请 |
| `services/api/adminRequests.ts` | ~150 | Admin Request API |
| `services/api/usage.ts` | ~100 | Usage API |

**总规模**：~487 行

**Qwen Code 现状**：Qwen Code 的 `/stats` 显示会话级统计，但**没有企业用量管理**。

**Qwen Code 修改方向**：
1. 新建 `commands/extra-usage/` 目录
2. 实现 `/extra-usage` 命令
3. （需要后端支持）DashScope Admin Request API
...
### 16. 交互式限速选项菜单 /rate-limit-options（P3）

**做什么**：Claude Code 的 `/rate-limit-options` 提供交互式菜单，帮助用户在遇到限速时选择操作：
...
| 文件 | 行数 | 职责 |
|------|------|------|
| `commands/rate-limit-options/rate-limit-options.tsx` | 209 | 限速选项菜单 UI |

**总规模**：~221 行

**Qwen Code 现状**：Qwen Code 的 API 客户端有重试逻辑，但**没有用户交互式的限速选项菜单**。

**Qwen Code 修改方向**：
1. 新建 `/rate-limit-options` 命令
2. 集成到限速错误处理流程
...
### 17. CustomSelect 自定义下拉选择（P3）

**做什么**：Claude Code 自定义下拉选择组件——支持搜索、键盘导航、多选、过滤：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `components/CustomSelect/select.tsx` | ~400 | 主组件——选项列表、搜索 |
| `components/CustomSelect/SelectMulti.tsx` | ~300 | 多选组件 |
| `components/CustomSelect/use-select-state.ts` | ~300 | 状态管理 |
| `components/CustomSelect/use-select-navigation.ts` | ~200 | 键盘导航 |
| `components/CustomSelect/use-select-input.ts` | ~250 | 搜索过滤 |

**总规模**：~3019 行

**Qwen Code 现状**：Qwen Code 使用 Ink 标准 `<Select>`——不支持搜索、多选。
...
### 18. Virtual Scrolling 虚拟滚动（P3）

**做什么**：Claude Code 的虚拟滚动——只渲染可视区域消息，支持 1000+ 轮流畅滚动：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/useVirtualScroll.ts` | 721 | 滚动计算、可视区域管理 |
| `ink/components/ScrollBox.tsx` | ~200 | 滚动容器 |

**总规模**：~921 行

**Qwen Code 现状**：Qwen Code 完整渲染所有消息——长对话时性能下降。

**Qwen Code 修改方向**：
1. 新建 `hooks/useVirtualScroll.ts`
2. 消息组件集成虚拟滚动
...
### 19. Feedback Survey 用户反馈调查（P3）

**做什么**：Claude Code 在关键操作后自动弹出反馈调查：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `components/FeedbackSurvey/FeedbackSurvey.tsx` | ~200 | 调查 UI |
...
### 20. Turn Diffs 轮次差异统计（P3）

**做什么**：Claude Code 为每个对话轮次计算差异统计——显示该轮修改了哪些文件：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/useTurnDiffs.ts` | 213 | 轮次差异计算、缓存 |

**总规模**：~213 行

**Qwen Code 现状**：Qwen Code 有 `git diff`，但**没有按轮次的差异统计**。

**Qwen Code 修改方向**：
1. 新建 `hooks/useTurnDiffs.ts`
2. 集成到消息组件
...
### 21. Session Backgrounding Ctrl+B 会话后台化（P3）

**做什么**：Claude Code 支持 **Ctrl+B** 快捷键将当前会话后台化：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/useSessionBackgrounding.ts` | 158 | 后台化/前台化切换 |

**总规模**：~158 行

**Qwen Code 现状**：Qwen Code 有 `--background` 启动后台任务，但**没有交互式 Ctrl+B 后台化**。

**Qwen Code 修改方向**：
1. 新建 `hooks/useSessionBackgrounding.ts`
2. 添加 Ctrl+B 快捷键绑定
...
**做什么**：Claude Code 的品牌标识系统——启动动画、欢迎信息、功能提示：
...
**Qwen Code 现状**：Qwen Code 有基础启动信息，但**没有品牌标识系统**。
...
**做什么**：Claude Code 的虚拟伴侣——终端内的 ASCII 精灵（鸭子），有稀有度、属性、帽子：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `buddy/sprites.ts` | 514 | 精灵定义——物种、稀有度、权重 |
| `buddy/CompanionSprite.tsx` | 370 | 渲染组件——ASCII 精灵 |
| `buddy/companion.ts` | 133 | 核心逻辑——Mulberry32 PRNG |
| `buddy/types.ts` | 148 | 类型定义 |
| `buddy/useBuddyNotification.tsx` | 97 | 通知 Hook |

**总规模**：1298 行

**Qwen Code 现状**：Qwen Code **完全没有伴侣/精灵系统**。
...
**做什么**：Claude Code 的 `remote/` 模块——本地 CLI 连接 CCR 云端 Agent：
...
| `remote/SessionsWebSocket.ts` | 404 | WebSocket 客户端——重连、心跳 |
| `remote/sdkMessageAdapter.ts` | 302 | SDK 消息适配 |
| `remote/remotePermissionBridge.ts` | 78 | 权限桥接 |

**总规模**：1127 行

**Qwen Code 现状**：Qwen Code **完全没有远程会话管理功能**。
...
**做什么**：Claude Code 的完整快捷键系统——解析、验证、加载用户自定义快捷键、冲突检测：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `keybindings/defaultBindings.ts` | 340 | 默认快捷键 |
| `keybindings/loadUserBindings.ts` | 472 | 加载用户自定义 |
| `keybindings/parser.ts` | 203 | 快捷键字符串解析 |
| `keybindings/resolver.ts` | 244 | 多 chord 状态机 |
| `keybindings/validate.ts` | 498 | 冲突检测 |
| `keybindings/schema.ts` | 236 | JSON Schema |
| `keybindings/KeybindingProviderSetup.tsx` | 307 | Provider 初始化 |

**总规模**：3159 行

**Qwen Code 现状**：Qwen Code 有基础快捷键（`keyBindings.ts` 186 行 + `keyMatchers.ts` 105 行 = **291 行**），但**没有用户自定义、multi-chord、冲突检测**。

**Qwen Code 修改方向**：
1. 新建 `keybindings/parser.ts`、`resolver.ts`、`validate.ts`、`schema.ts`
2. 支持 `~/.qwen/keybindings.json` 加载
...
### 26. useMoreRight 右面板扩展（P3）

**做什么**：Claude Code 的 `useMoreRight` Hook 管理右侧面板扩展：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `moreright/useMoreRight.tsx` | 25 | 右面板状态管理 |
