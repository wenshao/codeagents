...
### 24. CustomSelect 自定义下拉选择（P3）

**做什么**：Claude Code 自定义下拉选择组件——支持搜索、键盘导航、多选：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `components/CustomSelect/select.tsx` | ~400 | 主组件 |
...
**Qwen Code 修改方向**：新建 `components/CustomSelect/` 目录，实现搜索过滤、键盘导航、多选。
...
### 25. Session Backgrounding Ctrl+B 会话后台化（P3）
...
**Qwen Code 修改方向**：新建 `hooks/useSessionBackgrounding.ts`，添加 Ctrl+B 快捷键绑定。
...
### 26. 启动性能分析器 startupProfiler（P3）

**做什么**：Claude Code 内置启动性能分析器——测量各初始化阶段的耗时和内存占用：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `utils/startupProfiler.ts` | 194 | checkpoint 记录、内存快照 |
| `utils/profilerBase.ts` | ~100 | 基础分析函数 |

**总规模**：~294 行

**Qwen Code 现状**：Qwen Code **完全没有启动性能分析器**。

**Qwen Code 修改方向**：新建 `utils/startupProfiler.ts`，在启动关键点插入 `profileCheckpoint()` 调用。
...
### 27. Hook 文件变更监听 fileChangedWatcher（P3）
...
| 文件 | 行数 | 职责 |
|------|------|------|
| `utils/hooks/fileChangedWatcher.ts` | 191 | Hook 文件监听——chokidar + 防抖 |

**Qwen Code 现状**：Qwen Code 有 chokidar 用于 skill 文件监听，但**没有 hook 文件变更监听**。

**Qwen Code 修改方向**：新建 `utils/hooks/fileChangedWatcher.ts`，集成到 hook 加载流程。
...
**做什么**：Claude Code 的完整快捷键系统——解析、验证、加载用户自定义快捷键、冲突检测、多 chord 序列：
...
**Qwen Code 修改方向**：新建 `keybindings/parser.ts`、`resolver.ts`、`validate.ts`，支持 `~/.qwen/keybindings.json` 加载。
...
**做什么**：Claude Code 的 `remote/` 模块——本地 CLI 连接 CCR 云端 Agent，实现权限同步、消息转发：
...
| `remote/SessionsWebSocket.ts` | 404 | WebSocket 客户端 |
...
**Qwen Code 修改方向**：新建 `remote/` 模块，实现 WebSocket 客户端和权限桥接。
