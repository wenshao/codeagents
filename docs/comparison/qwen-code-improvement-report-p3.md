...
### 35. useAwaySummary 离开摘要 Hook（P3）

**做什么**：Claude Code 的 useAwaySummary Hook——用户离开后生成会话摘要：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/useAwaySummary.ts` | 125 | 离开摘要生成 |

**Qwen Code 现状**：Qwen Code **没有离开摘要功能**。

**Qwen Code 修改方向**：新建 `hooks/useAwaySummary.ts`。
...
### 36. useCommandKeybindings 命令快捷键（P3）

**做什么**：Claude Code 的 useCommandKeybindings Hook——命令快捷键管理：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/useCommandKeybindings.ts` | 107 | 命令快捷键 |

**Qwen Code 现状**：Qwen Code **没有命令快捷键 Hook**。

**Qwen Code 修改方向**：新建 `hooks/useCommandKeybindings.ts`。
...
### 37. usePluginRecommendationBase 插件推荐基础（P3）

**做什么**：Claude Code 的 usePluginRecommendationBase Hook——插件推荐基础逻辑：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/usePluginRecommendationBase.ts` | 104 | 插件推荐基础 |

**Qwen Code 现状**：Qwen Code **没有插件推荐基础 Hook**。

**Qwen Code 修改方向**：新建 `hooks/usePluginRecommendationBase.ts`。
...
### 38. useSkillImprovementSurvey 技能改进调查（P3）

**做什么**：Claude Code 的 useSkillImprovementSurvey Hook——技能改进反馈调查：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/useSkillImprovementSurvey.ts` | 105 | 技能改进调查 |

**Qwen Code 现状**：Qwen Code **没有技能改进调查功能**。

**Qwen Code 修改方向**：新建 `hooks/useSkillImprovementSurvey.ts`。
...
### 39. usePrStatus PR 状态 Hook（P3）

**做什么**：Claude Code 的 usePrStatus Hook——PR 状态查询：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/usePrStatus.ts` | 106 | PR 状态查询 |

**Qwen Code 现状**：Qwen Code **没有 PR 状态 Hook**。

**Qwen Code 修改方向**：新建 `hooks/usePrStatus.ts`。
...
### 40. useLogMessages 日志消息 Hook（P3）

**做什么**：Claude Code 的 useLogMessages Hook——日志消息管理：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/useLogMessages.ts` | 119 | 日志消息管理 |

**Qwen Code 现状**：Qwen Code **没有专门的日志消息 Hook**。

**Qwen Code 修改方向**：新建 `hooks/useLogMessages.ts`。
