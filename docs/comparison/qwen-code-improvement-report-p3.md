...
**做什么**：Claude Code 的 useInboxPoller Hook——轮询队友消息、权限请求、计划审批等：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/useInboxPoller.ts` | 969 | 收件箱轮询——队友消息、权限桥接、计划审批 |

**Qwen Code 现状**：Qwen Code **完全没有收件箱轮询机制**。

**Qwen Code 修改方向**：新建 `hooks/useInboxPoller.ts`。
...
### 27. extractMemories 记忆提取服务（P3）

**做什么**：Claude Code 的 extractMemories 服务——会话结束时自动提取持久记忆：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `services/extractMemories/extractMemories.ts` | 615 | 记忆提取核心逻辑 |
| `services/extractMemories/prompts.ts` | 154 | 提取 Prompt |

**总规模**：769 行

**Qwen Code 现状**：Qwen Code 有 `/memory` 工具，但**没有自动记忆提取服务**。

**Qwen Code 修改方向**：新建 `services/extractMemories/`。
...
### 28. useTypeahead 输入预填（P3）

**做什么**：Claude Code 的 useTypeahead Hook——输入时自动补全命令和路径：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/useTypeahead.ts` | 1384 | 输入预填——命令/路径补全 |

**Qwen Code 现状**：Qwen Code **没有输入预填功能**。

**Qwen Code 修改方向**：新建 `hooks/useTypeahead.ts`。
...
### 29. MagicDocs 自动文档维护（P3）

**做什么**：Claude Code 的 MagicDocs 服务——Agent 空闲时自动更新标记文档：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `services/MagicDocs/magicDocs.ts` | 254 | 自动文档更新核心 |
| `services/MagicDocs/prompts.ts` | 127 | 更新 Prompt |

**总规模**：381 行

**Qwen Code 现状**：Qwen Code **没有自动文档维护功能**。

**Qwen Code 修改方向**：新建 `services/MagicDocs/`。
