...
### 27. useAssistantHistory 代理历史 Hook（P3）

**做什么**：Claude Code 的 useAssistantHistory Hook——追踪和显示代理执行历史：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/useAssistantHistory.ts` | 250 | 代理历史追踪 |

**Qwen Code 现状**：Qwen Code **没有代理历史追踪功能**。

**Qwen Code 修改方向**：新建 `hooks/useAssistantHistory.ts`。
...
**做什么**：Claude Code 的 useBackgroundTaskNavigation Hook——键盘导航后台任务：
...
**Qwen Code 现状**：Qwen Code **没有后台任务键盘导航**。
...
**做什么**：Claude Code 的 useCancelRequest Hook——管理用户取消请求：
...
### 30. useDiffInIDE IDE 差异查看（P3）

**做什么**：Claude Code 的 useDiffInIDE Hook——在 IDE 中显示文件差异：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/useDiffInIDE.ts` | 379 | IDE 差异查看 |
...
### 31. AgentSummary 代理摘要服务（P3）

**做什么**：Claude Code 的 AgentSummary 服务——生成代理执行摘要：
