...
### 56. useHistorySearch 历史搜索 Hook（P3）

**做什么**：Claude Code 的 useHistorySearch Hook——搜索历史输入命令：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/useHistorySearch.ts` | 303 | 历史输入搜索、模糊匹配 |

**Qwen Code 现状**：Qwen Code 有基础历史导航，但**没有历史搜索功能**。

**Qwen Code 修改方向**：新建 `hooks/useHistorySearch.ts`。
...
### 57. useManagePlugins 插件管理 Hook（P3）

**做什么**：Claude Code 的 useManagePlugins Hook——管理插件市场、安装、卸载：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/useManagePlugins.ts` | 304 | 插件管理——市场浏览、安装、卸载 |

**Qwen Code 现状**：Qwen Code 有 skill 系统，但**没有插件管理 Hook**。

**Qwen Code 修改方向**：新建 `hooks/useManagePlugins.ts`。
...
### 58. useTurnDiffs 轮次差异 Hook（P3）

**做什么**：Claude Code 的 useTurnDiffs Hook——显示每轮对话的文件差异：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/useTurnDiffs.ts` | 213 | 轮次差异显示 |

**Qwen Code 现状**：Qwen Code 有 Turn Diffs 统计，但**没有 useTurnDiffs Hook**。

**Qwen Code 修改方向**：新建 `hooks/useTurnDiffs.ts`。
...
### 59. useGlobalKeybindings 全局快捷键 Hook（P3）

**做什么**：Claude Code 的 useGlobalKeybindings Hook——全局快捷键管理：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/useGlobalKeybindings.ts` | 248 | 全局快捷键 |

**Qwen Code 现状**：Qwen Code **没有全局快捷键 Hook**。

**Qwen Code 修改方向**：新建 `hooks/useGlobalKeybindings.ts`。
...
### 60. /terminalSetup 终端设置命令（P3）

**做什么**：Claude Code 的 /terminalSetup 命令——配置终端快捷键和多行输入：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `commands/terminalSetup/terminalSetup.tsx` | 553 | 终端设置 UI |

**Qwen Code 现状**：Qwen Code 有 terminalSetupCommand，但**功能较简单**（只配置 VS Code/Cursor/Windsurf）。

**Qwen Code 修改方向**：扩展现有 terminalSetupCommand，添加更多终端支持和交互配置。
