...
### 35. useSearchInput 输入搜索 Hook（P3）

**做什么**：Claude Code 的 useSearchInput Hook——输入框搜索功能：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/useSearchInput.ts` | 364 | 输入搜索 |

**Qwen Code 现状**：Qwen Code **没有输入搜索 Hook**。

**Qwen Code 修改方向**：新建 `hooks/useSearchInput.ts`。
...
### 36. usePasteHandler 粘贴处理 Hook（P3）

**做什么**：Claude Code 的 usePasteHandler Hook——处理粘贴事件（包括图片）：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/usePasteHandler.ts` | 285 | 粘贴事件处理 |

**Qwen Code 现状**：Qwen Code **没有专门的粘贴处理 Hook**。

**Qwen Code 修改方向**：新建 `hooks/usePasteHandler.ts`。
...
### 37. useGlobalKeybindings 全局快捷键 Hook（P3）
...
### 38. useSessionBackgrounding 会话后台化 Hook（P3）
...
### 39. useScheduledTasks 定时任务 Hook（P3）

**做什么**：Claude Code 的 useScheduledTasks Hook——管理定时任务：

**关键源码**：

| 文件 | 行数 | 职责 |
|------|------|------|
| `hooks/useScheduledTasks.ts` | 139 | 定时任务管理 |

**Qwen Code 现状**：Qwen Code **没有定时任务 Hook**。

**Qwen Code 修改方向**：新建 `hooks/useScheduledTasks.ts`。
