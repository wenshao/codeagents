...
> **"Silence is better than noise"**——GitHub 的核心设计理念是宁可不评论，也不产生噪声。29% 的审查**故意输出零评论**。
...
Copilot 代码审查从基于 prompt 的架构重构为**agentic tool-calling 架构**——Agent 主动收集仓库上下文（代码、目录结构、引用）来理解变更如何融入整体架构。升级后正面反馈增加 **8.1%**。
...
> "The deterministic gate is the test suite, not the review."
