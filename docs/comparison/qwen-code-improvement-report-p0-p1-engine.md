...
**相关进展（2026-04-24）**：本 item 是**纯 crash recovery**（自动检测 + 合成续行），与下面这些 session 管理 PR 是**互补关系**而非替代——用户主动回溯 ≠ 进程崩溃自动恢复。
...
崩溃自动恢复（本 item）仍需独立实现——上述 PR 都不覆盖"进程死了之后下次启动自动检测中断状态"这一核心能力。
...
| [PR#3471](https://github.com/QwenLM/qwen-code/pull/3471) | 🟡 OPEN | model-facing agent control（`task_stop` / `send_message` / per-agent transcript）—— 对标 Claude `TaskStop` + `SendMessage`，是 Worker 间通信的控制面 |
| [PR#3507](https://github.com/QwenLM/qwen-code/pull/3507) | 🟡 OPEN | **sticky todo panel in app layouts** —— 持久化 TODO 面板，对标 Claude `TodoWriteTool` UI，是任务状态可视化的前端入口 |
