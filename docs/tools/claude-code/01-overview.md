...
Claude Code 是 Anthropic 官方的 AI 编程代理，运行在终端中。核心为闭源 Rust 原生二进制（非 Node.js），通过原生安装脚本分发（npm 方式已废弃）。基于 Claude 系列模型（Sonnet 4.6、Opus 4.6、Haiku 4.5），拥有业界最大的 100 万 token 上下文窗口（Opus 4.6[1m]）。具备 20+ 内置工具（含延迟加载工具）、5 层设置优先级体系、24 种 Hook 事件、沙箱执行隔离、多代理协作、~79 个斜杠命令等能力。它也是唯一深度集成 Anthropic 模型的终端代理工具，在 SWE-bench 等复杂编程基准上表现领先。
...
- **Prompt Hook**：24 种事件类型，支持 LLM 推理驱动的 Hook 决策
...
4. **Prompt Hook**：24 种事件类型，LLM 推理决策（超越传统脚本 Hook）
...
- **Hook 系统**：24 种事件（含 UserPromptSubmit） 上下文注入
...
- [Hooks 参考](https://code.claude.com/docs/en/hooks) — 24 个事件完整列表
