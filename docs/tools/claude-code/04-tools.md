...
>
> **计数规则**：38 = 10 核心（始终加载）+ 24 延迟（ToolSearch 按需加载）+ 3 内部 + 1 条件（Windows PowerShell）。不含 MCP 动态工具（数量由 MCP 服务器决定）。其中 TaskStop 含 KillShell 别名，Edit 含 replace_all 批量编辑模式，均非独立工具。用户常见/常驻工具约 10 个（核心工具），其余按功能需求动态激活。
...
**安全变量**（41 个，自动从命令前缀中剥离后再做权限匹配）：Go、Rust、Node、Python、pytest 相关、`ANTHROPIC_API_KEY`、locale、终端、颜色配置等。
...
### 4.10.1 WebFetch 工具（源码: `tools/WebFetchTool/`，1,131 LOC）
...
### 4.10.2 WebSearch 工具（源码: `tools/WebSearchTool/`，569 LOC）
...
### 4.10.3 NotebookEdit 工具（源码: `tools/NotebookEditTool/`，587 LOC）
...
### 4.10.5 Cron 工具（源码: `tools/ScheduleCronTool/`）
...
### 4.10.7 Brief 工具（源码: `tools/BriefTool/`，610 LOC）
...
### 4.10.8 AskUserQuestion 工具（源码: `tools/AskUserQuestionTool/`，309 LOC）
...
### 4.10.9 SendMessage 工具（源码: `tools/SendMessageTool/`，997 LOC）
...
### 4.10.11 LSP 工具（源码: `tools/LSPTool/`，2,005 LOC）
...
### 4.10.15 Skill 工具（源码: `tools/SkillTool/`，1,477 LOC）
...
## 4.11 MCP 工具集成（源码: `tools/MCPTool/`）
