# Claude Code 命令详解（v2.1.81）

> 基于 v2.1.81 二进制分析。Claude Code 共有四种命令类型、约 60 个斜杠命令。本文档逐一说明每个命令的功能、实现方式和使用示例。
...
**实现细节：**
- 显示当前会话 ID、创建时间、token 使用量等信息
- 可用于分享会话 ID 以便在其他终端恢复
...
- **类型：** local-jsx
- **功能：** 管理 Prompt Hook 配置（10 种事件类型的钩子函数）
- **别名：** 无
...
- **类型：** local-jsx
- **功能：** 配置桌面应用集成（Claude Desktop App）
- **别名：** 无
...
- **类型：** local-jsx
- **功能：** 配置移动端集成（通过 claude.ai 移动应用远程控制）
- **别名：** 无
...
- **类型：** local-jsx
- **功能：** 管理 Claude Code 的安装配置（shell 集成、PATH 设置等）
- **别名：** 无
...
- 检查当前版本与最新版本
- 渲染升级确认 UI
- 执行 `curl install.sh | bash` 进行原地升级
...
- **类型：** local-jsx
- **功能：** 管理 Claude Code 的 Pass（通行证/配额包）
- **别名：** 无
...
- **类型：** local-jsx
- **功能：** 回溯到之前的思考状态（类似 checkpoint rewind）
- **别名：** 无
...
**实现细节：**
- 创建一个定时循环，按指定间隔重复执行目标命令
- 默认间隔为 10 分钟
- 适合持续监控场景：定期审查代码、定期提交等
- 底层使用 Cron 机制实现
- 可通过 Ctrl+C 或 `/tasks` 管理来停止
...
- **类型：** skill
- **功能：** 创建和管理 cron 定时调度的远程 Agent 任务
- **别名：** 无

**使用示例：**
```
/schedule
/schedule create "每天早上审查 PR" --cron "0 9 * * *"
```

**实现细节：**
- 通过 CronCreate / CronList / CronDelete 工具管理定时任务
- 任务在远程服务器上由独立的 Agent 实例执行
- 每个任务有独立的 worktree 和上下文
- 支持标准 cron 表达式定义执行频率
- 任务结果通过 Notification 推送
...
| `/remote-control` | local-jsx | 远程控制模式 |
| `/remote-env` | local-jsx | 远程环境设置 |
| `/desktop` | local-jsx | 桌面应用集成 |
| `/mobile` | local-jsx | 移动端集成 |
| `/install-github-app` | local-jsx | 安装 GitHub App |
| `/web-setup` | local-jsx | Web 集成设置 |
| `/exit` | local-jsx | 退出会话 |
| `/install` | local-jsx | 安装配置 |
| `/upgrade` | local-jsx | 升级版本 |
| `/terminal-setup` | local-jsx | 终端环境设置 |
| `/passes` | local-jsx | 管理 Pass 配额 |
| `/think-back` | local-jsx | 回溯思考状态 |
| `/branch` | local-jsx | 管理 Git 分支 |
