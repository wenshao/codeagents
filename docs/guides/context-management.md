...
> Claude Code 和 Gemini CLI 拥有固定 1M 窗口，其他工具取决于用户选择的模型。
...
3. **Prompt Caching** —— `cache_control: ephemeral`（65 处引用），缓存系统提示以减少重复 token 消耗
...
| 工具 | 缓存机制 | 证据 |
|------|---------|------|
| **Claude Code** | Anthropic `cache_control: ephemeral` | 二进制中 65 处引用 |
